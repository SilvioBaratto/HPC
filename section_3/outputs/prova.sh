...
echo 'type,#procs,nx,ny,nz,comm_level,t_sweep[sec],P_prg[MLUPs],T_tot[sec]' > results/jacobi_data.csv
...
do 
    ...
    avg=$(cat tmp_data | sed 's/,/ /g' | awk '{$6=($3+$4)*0.5};1' | awk '{sum+=$6; sum2+=$5; sum3+=$1} END {print sum/10; print sum2/10; print sum3/10}')
